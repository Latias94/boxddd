...
| Shapes | Inclined Plane, Rolling Resistance, High Resistance, Isotropic Friction, Slide Twist, Restitution, Static Invoke, Conveyor Belt, Conveyor Mesh, Wind, Wind Drop, Wind Flap | `boxddd/examples/hello_world.rs`, `boxddd/examples/advanced_collision.rs`, `bevy_boxddd/examples/advanced_colliders_3d.rs`, `bevy_boxddd/examples/testbed_3d` materials scene | Partial | Bevy materials scene covers visible friction and restitution with today's plugin surface. Rolling resistance, conveyor, and wind-style fields are not exposed through `bevy_boxddd` yet. |
