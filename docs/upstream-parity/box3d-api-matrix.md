...
| Body attached resources/events/queries | body shapes, joints, contacts, AABB, closest point, ray/shape overlap/casts, collide mover | Wrapped/Deferred | Shapes/joints/contacts/AABB, body-local closest point, ray casts, shape casts, and overlap are wrapped; collide mover remains deferred. |
...
| Shape lifecycle/runtime | destroy, valid, type, body/world, sensor, density, friction, restitution, material, filter, event toggles, AABB, mass data, geometry getters/setters | Wrapped | Mesh material indexing is wrapped with bounds checks. |
| Shape user data/wind | `b3Shape_SetUserData`, `b3Shape_GetUserData`, `b3Shape_ApplyWind` | Raw-only/Deferred | Pointer user data stays raw; wind needs behavior tests before safe exposure. |
| Shape contacts/sensors/query helpers | `b3Shape_GetContactData`, `b3Shape_GetSensorData`, `b3Shape_RayCast`, closest point | Wrapped/Deferred | AABB, mass data, direct ray-cast, and closest-point helpers are wrapped; shape contact/sensor data and advanced geometry readback remain deferred. |
